...
The core implementation of the project is located in `src/mapemgen/`. This folder contains the main pipeline code for extracting site information, assigning geometry and semantics, matching evidence to MAPEM fields, fusing evidence, generating MAPEM-style outputs and producing validation or review reports.

The `data/` folder contains the site data used during the project. The `docs/` folder contains project notes, related materials and user-facing documentation, including the pipeline user guide. The `notes/` folder is used for development notes and intermediate working materials. Configuration files are stored in `configs/`, while the top-level runner scripts, `run_pipeline.py` and `run_pipeline_with_generator.py`, provide convenient entry points for running the workflow.
