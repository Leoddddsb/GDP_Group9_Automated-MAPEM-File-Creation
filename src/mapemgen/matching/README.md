...
```
MAPEM_Dictionary_v4_clean.xlsx + generator_overlay.json ─(build_rules_from_dict_v3.py)─▶ matching_rules.yaml ┐
                                                                                  │
...
(unwraps payloads, normalises confidence, injects instance keys). It also routes
phase/label facts to a lane: assignment emits `movement_lane_mappings`
({movement_ref, lane_ref, phase_refs[], requires_context_match}); the adapter
indexes it by movement_ref and phase_ref and resolves each phase fact's ref to a
lane_ref (mappings flagged `requires_context_match` are injected but marked
provisional). A `--movement-map` file can override or fill gaps.
...
python build_rules_from_dict_v3.py MAPEM_Dictionary_v4_clean.xlsx generator_overlay.json matching_rules.yaml
...
# normalisation (but no geometry instance keys). When present it also supplies
# movement_lane_mappings, which route phase facts to lanes. An optional
# --movement-map movement_to_lane.json can override / fill gaps.
...
| File | Role | Edit? |
|---|---|---|
| `MAPEM_Dictionary_v4_clean.xlsx` | source of truth: facts + priority (team maintains) | ✎ team |
...
**Add / change a fact or its priority** → edit `MAPEM_Dictionary_v4_clean.xlsx`, rerun
