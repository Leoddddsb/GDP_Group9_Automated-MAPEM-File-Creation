...
def _approach_lookup(value, resolved=None, scope=None, want_dir=None, **_ignore):
    ...
    maps lane_ref -> {id, dir, arm}. Return the approach id, but only if the
    lane's driving direction matches want_dir ('ingress'/'egress').

    [STANDARD: C-Roads 3.2.0 section 3.3.1 — IngressApproach/egressApproach]
    "...at least one approach PER intersection arm AND PER driving direction."
    The upstream approach_ref carried by the fact payload (e.g. 'approach_4')
    does NOT respect driving direction: the assignment step can place an
    ingress lane and an egress lane of the same arm into the SAME approach_ref,
    which yields one approach id used for both ingress and egress (invalid).
    The _analyze_lanes prepass re-derives ids split by (arm, direction) and
    stores them in resolved['approach']. We therefore look up the prepass id
    FIRST (authoritative), and only fall back to the fact's own approach_ref
    when the prepass has no entry for this lane."""
    ...
    # AUTHORITATIVE: prepass id, already split by (arm, direction)
    if lane_ref and lane_ref in appr:
        info = appr[lane_ref]
        if want_dir and info.get("dir") != want_dir:
            return None      # this lane is not of the requested direction
        return info.get("id")

    # FALLBACK only (prepass had no geometry for this lane): trust the fact's
    # own direction + approach_ref. Still gated by want_dir.
    if isinstance(value, Mapping):
        direction = str(value.get("direction") or "").lower()
        if direction:
...
# --- lane_type_from_label override (fix over-broad "cross" match) ------------
# [STANDARD: C-Roads 3.2.0 section 3.3.2.1 — LaneType]
# Base lane_type_from_label matched the bare substring "cross", classifying
# ordinary traffic lanes as crosswalkLane whenever their label/layer text
# contained "cross" inside another word (e.g. "crossroad", "crossover lane",
# and CAD tactile-paving facts of semantic_type "crossing_or_tactile" on the
# "Pr-Tactiles" layer). That produced crosswalk lanes carrying vehicle turn
# connections — a contradiction. Use precise pedestrian-crossing terms instead.
import re as _re
_CROSSWALK_RE = _re.compile(
    r"\b(?:cross\s*walk|cross\s*ing|zebra|puffin|toucan|pelican|pegasus)\b",
    _re.IGNORECASE)
_CROSSWALK_TERMS = ("crosswalk", "crossing", "zebra", "puffin", "toucan",
                    "pelican", "pegasus")


def lane_type_from_label(value, output="sitemodel", **_ignore):
    text = str(value or "").lower()
    if _CROSSWALK_RE.search(text) or any(t in text for t in _CROSSWALK_TERMS):
        return "crosswalkLane" if output == "mapem" else "crosswalk"
    if any(k in text for k in ("cycle", "bike")):
        return "bikeLane"
    if any(k in text for k in ("footway", "pavement", "sidewalk")):
        return "sideWalk" if output == "mapem" else "sidewalk"
    if any(k in text for k in ("median", "island", "central reserve")):
        return "medianLane" if output == "mapem" else "median"
    if any(k in text for k in ("tram", "lrt", "rail", "tracked")):
        return "trackedVehicle"
    if any(k in text for k in ("parking", "bay")):
        return "parking"
    return "vehicle"


def layer_name_to_lanetype(value, output="sitemodel", **_ignore):
    return lane_type_from_label(value, output=output)


TRANSFORMS["lane_type_from_label"] = lane_type_from_label
TRANSFORMS["layer_name_to_lanetype"] = layer_name_to_lanetype
TRANSFORMS["layer_or_marking_to_lane_type"] = lane_type_from_label
