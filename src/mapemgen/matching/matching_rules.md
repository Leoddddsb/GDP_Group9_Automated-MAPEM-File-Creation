...
selects evidence by a **weighted final score** (v3) that combines extraction
confidence, conflict agreement, and source priority (P1–F), preserving
full source provenance and conflict information, and outputs `mapped_evidence.json`
for downstream evidence fusion. Confidence is folded into selection (the
`scoring` section), not a separate stage.
