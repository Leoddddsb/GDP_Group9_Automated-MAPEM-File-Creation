...
> In v3 this agreement information is **part of the selection score** itself
> (`conflict_agreement` is one of the three weighted terms in `final_score`),
> and is also exported per field for downstream fusion.
>
...
1. Scores each candidate by `final_score` (confidence + agreement + priority) and
   picks the **highest-scoring** value.
