...
class MatchingEngine:
    ...
    def _analyze_lanes(self, facts, ctx):
        ...
        try:
            ...
            # "Unidirectional lanes shall be grouped into approaches ... at least
            # one approach PER intersection arm AND PER driving direction."
            # cluster_by_direction groups by BEARING (the arm) only, so the same
            # arm's ingress and egress lanes share a bearing cluster and would get
            # the SAME approach id (invalid: one id used for both ingress and
            # egress). Split by (bearing-cluster, direction) and assign a stable,
            # unique, contiguous integer id per group.
            raw = []
            ...
            def _gid(key):
                if key not in group_id:
                    group_id[key] = len(group_id) + 1
                return group_id[key]
            for r, cid, d in sorted(raw, key=lambda x: (x[2] or "", x[1])):
                _gid((cid, d))
            appr = {}
            for r, cid, d in raw:
                appr[r] = {"id": _gid((cid, d)), "dir": d, "arm": cid}
    ...
    def _select_by_score(self, candidates, rule, ctx, eff_priority):
        ...
        if rule["target"].endswith(".laneAttributes.directionalUse"):
            # guard: only compare hashable (string) values; some candidates carry
            # a dict payload as value, which would raise on `in {...}`.
            explicit = [c for c in selectable
                        if isinstance(c["value"], str) and c["value"] in {"10", "01", "11"}]
            if explicit:
                selectable = explicit
