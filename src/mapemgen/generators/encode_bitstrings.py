...
def encode_directional_use(value: Any):
    ...
    if value is None:
        return None
    # Already a 2-bit string? The matching stage may emit "10"/"01"/"11"/"00"
    # directly. Pass a valid bit string straight through (idempotent encoding) so
    # the field is not double-encoded.
    if isinstance(value, str) and value in ("00", "01", "10", "11"):
        if BITSTRING_AS_INT:
            return int(value[::-1] if not REVERSE_BITS else value, 2)
        return value if not REVERSE_BITS else value[::-1]
...
# ===========================================================================
# Accept BOTH the J2735 bit names and the semantic values the matching stage
# actually emits (straight / leftTurn / rightTurn / uTurn). Keys are matched
# case-insensitively with spaces/hyphens stripped (see encode_maneuver).
ALLOWED_MANEUVER_BITS = {
    "maneuverstraightallowed": 0, "straight": 0, "through": 0,
    "maneuverleftallowed": 1, "left": 1, "leftturn": 1,
    "maneuverrightallowed": 2, "right": 2, "rightturn": 2,
    "maneuveruturnallowed": 3, "uturn": 3, "uturnallowed": 3,
...
# Semantic values that mean "no/unknown manoeuvre". These are NOT errors: the
# matching stage emits "unknown" when it cannot determine the turn from geometry.
# Per C-Roads, an unresolved maneuver should be left absent (a gap), not encoded
# as a bogus bit. Encode these as None so the field stays empty/optional.
_MANEUVER_NONE_VALUES = {"unknown", "none", "undetermined", ""}
...
def encode_maneuver(value: Any):
    ...
    bits: set[int] = set()
    for item in value:
        key = str(item).strip().lower().replace(" ", "").replace("-", "").replace("_", "")
        # "unknown"/"none" mean the manoeuvre is undetermined -> leave absent
        if key in _MANEUVER_NONE_VALUES:
            continue
        if key not in ALLOWED_MANEUVER_BITS:
            raise EncodeError(f"maneuver: unknown value {item!r}")
        bits.add(ALLOWED_MANEUVER_BITS[key])
    if not bits:
        return None          # nothing resolvable -> field stays empty (a gap)
