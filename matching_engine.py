...
class MatchingEngine:
    ...
    def _process_one(self, rule, path, scope, facts, ctx, priority_overrides):
        ...
        if mode == "client_configured":
            ...
            rec.value = ctx["config"].get(key) if key else None
            rec.confidence = "high"
    ...
    def _process_sources(self, rule, rec, scope, facts, ctx, priority_overrides):
        ...
        # 2. WITHIN each logical group: OR-select the best source by priority
        ...
        transforms = self._pipeline_for(rec.target_path, primary_fact_name)
                ...
                if not pending:
                    try:
                        ...
                        cv, _ = self.transforms.run(
                            self._pipeline_for(target, fn), fct.payload, ctx)
                    except Exception:
                        cv = None
